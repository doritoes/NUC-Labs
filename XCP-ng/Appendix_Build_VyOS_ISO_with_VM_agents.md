...
- `sudo docker run --rm -it --privileged -v $(pwd):/vyos -w /vyos vyos/vyos-build:current bash`
  ...
  - `exit`
- When the build is successful, the resulting iso can be found inside the build directory similar to vyos-1.5-rolling-202510072128-generic-amd64. On the host fint it in  `~/vyos-build/build`.
